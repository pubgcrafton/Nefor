...
class HikkaBackupMod(loader.Module):
    ...
    async def client_ready(self, client, db):
        ...
        if not self.get("period"):
            await self.inline.bot.send_photo(
                self._tg_id,
                photo="https://imgur.com/mjMrU2h",
                caption=self.strings("period"),
                reply_markup=self.inline.generate_markup(
                    utils.chunks(
                        [
                            {"text": f"▫️ {i} h", "data": f"backup_period/{i}"}
                            for i in {1, 2, 4, 6, 8, 12, 24, 48, 168}
                        ],
                        3,
                    )
                    + [[{"text": "▪️ Never", "data": "backup_period/never"}]]
                ),
            )
    ...
    async def handler(self):
        try:
            ...
            backup.name = f"nino-db-backup-{getattr(datetime, 'datetime', datetime).now().strftime('%d-%m-%Y-%H-%M')}.json"
