...
class SearcherMod(loader.Module):
    ...
    strings = {
        "name": "Ninolinks",
        "yutub": "🫂 <b>YouTube link special for you.</b>\n\n",
        ...
        if await self.allmodules.check_security(
            ...
            try:
                await self.inline.form(
                    ...
                    reply_markup=[
                        [{"text": "♨️ Link", "url": f"https://www.google.com/search?q={text}"}],
                        [{"text": "🔻 Close", "action": f"close"}],
                    ],
        ...
        if await self.allmodules.check_security(
            ...
            try:
                await self.inline.form(
                    ...
                    reply_markup=[
                        [{"text": "♨️ Link", "url": f"https://github.com/search?q={text}"}],
                        [{"text": "🔻 Close", "action": f"close"}],
                    ],
        ...
        if await self.allmodules.check_security(
            ...
            try:
                await self.inline.form(
                    ...
                    reply_markup=[
                        [{"text": "♨️ Link", "url": f"https://rt.pornhub.com/video/search?search={text}"}],
                        [{"text": "🔻 Close", "action": f"close"}],
                    ],
